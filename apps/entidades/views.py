...
class RegistrarPerfil(LoginRequiredMixin, View):
	...
	def post(self, request, *args, **kwargs):
		...
		if action == 'nuevo_usuario':
			...
			# Verificar si ya existe un usuario con el mismo nombre de usuario (cedula)
			if User.objects.filter(username=username).exists():
				data['response'] = {'title': 'Ocurrió un error!', 'data': 'Ya existe un usuario con esta cédula.', 'type_response': 'danger'}
				return JsonResponse(data, safe=False)

			# Verificar si ya existe un usuario con el mismo correo electrónico (email)
			if User.objects.filter(email=email).exists():
				data['response'] = {'title': 'Ocurrió un error!', 'data': 'Ya existe un usuario con este correo electrónico.', 'type_response': 'danger'}
				return JsonResponse(data, safe=False)

			# Verificar si ya existe un usuario con el mismo número de teléfono
			if Perfil.objects.filter(telefono=telefono).exists():
				data['response'] = {'title': 'Ocurrió un error!', 'data': 'Ya existe un usuario con este teléfono.', 'type_response': 'danger'}
				return JsonResponse(data, safe=False)
			...
			if Beneficiado.objects.filter(cedula=perfil.cedula).exists():
				beneficiado = Beneficiado.objects.get(cedula=perfil.cedula)
			else:
				beneficiado = Beneficiado.objects.create(
					perfil=perfil,
					nacionalidad=request.POST["nacionalidad"],
					cedula=request.POST["cedula"],
					nombres=request.POST["nombres"],
			...
			html_content = render_to_string('email/email_registro.html', {'correo': email,'user': f'{request.POST["nacionalidad"]}{request.POST["cedula"]}','nombres': request.POST['nombres'], 'apellidos': request.POST['apellidos']})
			subject, from_email, to = 'REGISTRO EXITOSO', 'FARMACIA COMUNITARIA ASIC LEONIDAS RAMOS', email
			text_content = 'ESTE ES UN MENSAJE DE BIENVENIDA.'
			EmailThread(subject, text_content, from_email, [to], False, html_content).start()

			data['response'] = {'title': 'Éxito!', 'data': 'Usuario creado correctamente.', 'type_response': 'success'}
    ...
    def post(self, request, *args, **kwargs):
        ...
        if action == 'nuevo_bene':
            ...
            if not Beneficiado.objects.filter(cedula=request.POST['cedula']).exists():
                ...
                # Validar fecha de nacimiento si el parentesco es hijo o hija
                if bene.parentesco in ['HO', 'HA']:  # Si es hijo o hija
                    perfil = Perfil.objects.get(usuario__username=self.request.user.username)
                    fecha_nacimiento_perfil = perfil.f_nacimiento
                    fecha_nacimiento_bene = datetime.strptime(bene.f_nacimiento, '%Y-%m-%d').date()

                    if fecha_nacimiento_bene < fecha_nacimiento_perfil:
                        data['response'] = {'title': 'Ocurrió un error!',
                                            'data': 'La fecha de nacimiento del beneficiado no puede ser menor que la del titular.',
                                            'type_response': 'danger'}
                        return JsonResponse(data, safe=False)

                if 'genero' in request.POST and 'parentesco' in request.POST:
                    genero = request.POST['genero']
                    parentesco = request.POST['parentesco']

                    if genero == 'MA' and parentesco in ['HA', 'EA']:
                        data['response'] = {'title': 'Error', 'data': 'No puedes seleccionar "Hija" o "Esposa" para un beneficiario masculino.', 'type_response': 'danger'}
                        return JsonResponse(data, safe=False)

                    if genero == 'FE' and parentesco in ['HO', 'EO']:
                        data['response'] = {'title': 'Error', 'data': 'No puedes seleccionar "Hijo" o "Esposo" para un beneficiario femenino.', 'type_response': 'danger'}
                        return JsonResponse(data, safe=False)

                bene.save()
                data['response'] = {'title': 'Éxito!', 'data': 'Beneficiado registrado correctamente.',
                                    'type_response': 'success'}

            else:
                data['response'] = {'title': 'Ocurrió un error!', 'data': 'Beneficiado ya se encuentra registrado.',
                                    'type_response': 'danger'}
        ...
        if action == 'editar_bene':
            ...
            data['response'] = {'title': 'Éxito!', 'data': 'Beneficiado registrado correctamente.', 'type_response': 'success'}

        # Validar el género y el parentesco
        #if 'genero' in request.POST and 'parentesco' in request.POST:
        #    genero = request.POST['genero']
        #    parentesco = request.POST['parentesco']

        #    if genero == 'MA' and parentesco in ['HA', 'EA']:
        #        data['response'] = {'title': 'Error', 'data': 'No puedes seleccionar "Hija" o "Esposa" para un beneficiario masculino.', 'type_response': 'error'}
        #        return JsonResponse(data, safe=False)

        #    if genero == 'FE' and parentesco in ['HO', 'EO']:
        #        data['response'] = {'title': 'Error', 'data': 'No puedes seleccionar "Hijo" o "Esposo" para un beneficiario femenino.', 'type_response': 'error'}
        #        return JsonResponse(data, safe=False)

        if action == 'eliminar_bene':
            beneficiario_pk = request.POST.get('id')
            try:
                beneficiario = Beneficiado.objects.filter(pk=beneficiario_pk).first()
                if beneficiario:
                    beneficiario.delete()
                    data['response'] = {'title': 'Éxito!', 'data': 'Beneficiario eliminado correctamente.', 'type_response': 'success'}
                else:
                    data['response'] = {'title': 'Error', 'data': 'El beneficiario no existe.', 'type_response': 'error'}
            except Beneficiado.DoesNotExist:
                data['response'] = {'title': 'Error', 'data': 'El beneficiario no existe.', 'type_response': 'error'}
